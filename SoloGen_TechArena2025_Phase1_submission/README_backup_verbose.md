...
**SoloGen Team - Phase 1 Submission**
...
- **Optional (if available):** CPLEX or Gurobi (commercial solvers, faster)
- **Auto-detection:** The system automatically detects and uses the best available solver
  - Priority: CPLEX → Gurobi → HiGHS → CBC → GLPK
...
This will:
1. Load market data from `input/TechArena2025_data_tidy.jsonl`
2. Run optimization for all 45 scenarios (5 countries × 3 C-rates × 3 cycle limits)
3. Generate three output files in `output/` folder:
   - `TechArena_Phase1_Configuration.xlsx`
   - `TechArena_Phase1_Investment.xlsx`
   - `TechArena_Phase1_Operation.xlsx`

**Expected runtime:** ~20-30 minutes for all 45 scenarios (depends on solver and hardware)

## Output Files Structure

### 1. Configuration.xlsx
Contains 5 country sheets (DE, AT, CH, HU, CZ), each with 9 scenarios:
- C-rate: 0.25, 0.33, 0.50
- number of cycles: 1.0, 1.5, 2.0
- yearly profits [kEUR/MW]
- levelized ROI [%]

### 2. Investment.xlsx
Contains 5 country sheets with DCF analysis for the best scenario per country:
- Financial parameters (WACC, inflation rate, discount rate)
- 10-year cash flow projection (2023-2033)
- Levelized ROI calculation

### 3. Operation.xlsx
Contains 5 country sheets with full year (35,136 time steps) operational schedule:
- Timestamp (15-minute intervals)
- Stored energy [MWh], SoC [-]
- Charge [MWh], Discharge [MWh]
- Day-ahead buy/sell [MWh]
...
├── model.py                   # ImprovedBESSOptimizer class
├── investment_analysis.py     # InvestmentAnalyzer class
├── excel_generator.py         # Output file generation functions
├── requirements.txt           # Python dependencies
├── README.md                  # This file
├── input/
│   └── TechArena2025_data_tidy.jsonl  # Market price data
