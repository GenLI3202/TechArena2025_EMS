# TechArena 2025 Phase 1: BESS Energy Management System# Huawei TechArena 2025: BESS Energy Management System



**SoloGen Team Submission****SoloGen Team - Phase 1 Submission**



## Quick StartA Python-based Energy Management System (EMS) for optimizing Battery Energy Storage System (BESS) operations across multiple European electricity markets.
...
### Run Optimizationpip install -r requirements.txt
...
- **Recommended for competition:** HiGHS (open-source, included in requirements.txt)

This generates three output Excel files in `output/` folder:- **Optional (if available):** CPLEX or Gurobi (commercial solvers, faster)

- `TechArena_Phase1_Configuration.xlsx` - Configuration analysis (9 scenarios per country)- **Auto-detection:** The system automatically detects and uses the best available solver

- `TechArena_Phase1_Investment.xlsx` - Investment analysis with DCF (best scenario per country)  - Priority: CPLEX → Gurobi → HiGHS → CBC → GLPK

- `TechArena_Phase1_Operation.xlsx` - Full year operational schedule (35,136 time steps)

### Running the Optimization

**Runtime:** ~20-30 minutes (45 scenarios: 5 countries × 3 C-rates × 3 cycle limits)

```bash

## Technical Approachpython main.py

```

### Optimization Model

- **Framework:** Pyomo with auto-detected solver (HiGHS/CPLEX/Gurobi)This will:

- **Objective:** Maximize annual revenue from day-ahead arbitrage and ancillary services (FCR, aFRR)1. Load market data from `input/TechArena2025_data_tidy.jsonl`

- **Constraints:** Battery SOC dynamics, power limits, market rules, daily cycle limits2. Run optimization for all 45 scenarios (5 countries × 3 C-rates × 3 cycle limits)

- **Time resolution:** 15-minute intervals (35,136 time steps for 2024)3. Generate three output files in `output/` folder:

   - `TechArena_Phase1_Configuration.xlsx`

### Investment Analysis   - `TechArena_Phase1_Investment.xlsx`

- **Method:** Discounted Cash Flow (DCF) analysis   - `TechArena_Phase1_Operation.xlsx`

- **Horizon:** 10 years (2023-2033)

- **Metrics:** NPV, IRR, Levelized ROI**Expected runtime:** ~20-30 minutes for all 45 scenarios (depends on solver and hardware)

- **Factors:** Country-specific WACC, inflation rates, battery degradation (2.5%/year)

## Output Files Structure

### BESS Configuration

- **Capacity:** 4,472 kWh (Huawei LUNA2000-4.5MWh)### 1. Configuration.xlsx

- **Efficiency:** 95% (charge/discharge)Contains 5 country sheets (DE, AT, CH, HU, CZ), each with 9 scenarios:

- **C-rates:** 0.25, 0.33, 0.50 (max power: 1,118-2,236 kW)- C-rate: 0.25, 0.33, 0.50

- **Cycle limits:** 1.0, 1.5, 2.0 cycles/day- number of cycles: 1.0, 1.5, 2.0

- **Investment cost:** 200 EUR/kWh- yearly profits [kEUR/MW]

- levelized ROI [%]

### Markets Optimized

1. **Day-Ahead (EPEX SPOT):** Energy arbitrage, 15-min intervals### 2. Investment.xlsx

2. **FCR (Frequency Containment Reserve):** Symmetric capacity bids, 4-hour blocksContains 5 country sheets with DCF analysis for the best scenario per country:

3. **aFRR (Automatic Frequency Restoration Reserve):** Asymmetric capacity bids, 4-hour blocks- Financial parameters (WACC, inflation rate, discount rate)

- 10-year cash flow projection (2023-2033)

### Countries Analyzed- Levelized ROI calculation

- **DE** (Germany/Luxembourg): Coupled day-ahead market

- **AT** (Austria)### 3. Operation.xlsx

- **CH** (Switzerland)Contains 5 country sheets with full year (35,136 time steps) operational schedule:

- **HU** (Hungary)- Timestamp (15-minute intervals)

- **CZ** (Czech Republic)- Stored energy [MWh], SoC [-]

- Charge [MWh], Discharge [MWh]

## Project Structure- Day-ahead buy/sell [MWh]

- FCR, aFRR Capacity [MW]

```

├── main.py                    # Main execution script## Project Structure

├── model.py                   # Optimization model (ImprovedBESSOptimizer)

├── investment_analysis.py     # DCF analysis (InvestmentAnalyzer)```

├── excel_generator.py         # Output file generatorsSoloGen_TechArena2025_Phase1_submission/

├── requirements.txt           # Dependencies (HiGHS solver included)├── main.py                    # Main execution script

├── validate_submission.py     # Testing script (optional)├── model.py                   # ImprovedBESSOptimizer class

├── input/├── investment_analysis.py     # InvestmentAnalyzer class

│   └── TechArena2025_data_tidy.jsonl  # Market price data (preprocessed)├── excel_generator.py         # Output file generation functions

└── output/                    # Generated Excel files├── requirements.txt           # Python dependencies

```├── README.md                  # This file

├── input/

## Key Implementation Details│   └── TechArena2025_data_tidy.jsonl  # Market price data

└── output/                    # Generated files (created on run)

### Solver Configuration    ├── TechArena_Phase1_Configuration.xlsx

- **Auto-detection:** CPLEX → Gurobi → HiGHS → CBC → GLPK    ├── TechArena_Phase1_Investment.xlsx

- **Required:** HiGHS (open-source, included in requirements.txt)    └── TechArena_Phase1_Operation.xlsx

- **Optional:** CPLEX/Gurobi (commercial, if available)```



### Country Mapping## Table of Contents

- Optimization uses 'DE_LU' for coupled Germany-Luxembourg market

- Excel output uses 'DE' as required by competition format- [Quick Start](#quick-start)

- Mapping handled automatically in `excel_generator.py`- [Challenge Context](#challenge-context)

- [Mathematical Model](#mathematical-model)

### Solution Extraction- [Investment Optimization](#investment-optimization)

- JSON serialization converts dict keys to strings- [Implementation Pipeline](#implementation-pipeline)

- All indices converted to strings before lookup (`str(t)`, `str(block_id)`)- [Pyomo Framework Overview](#pyomo-framework-overview)

- Ensures correct extraction of charge, discharge, and bid values- [Key Concepts](#key-concepts)



### Data Processing---

- Full year: 35,136 time steps (366 days × 96 intervals/day for 2024 leap year)

- Day-ahead prices: 15-minute resolution## Challenge Context

- Ancillary service prices: 4-hour block resolution

- Forward-fill for missing valuesThe primary objective is to develop a Python-based Energy Management System (EMS) algorithm for a utility-scale Battery Energy Storage System (BESS). The goal is to optimize the BESS's financial performance by participating in multiple European electricity markets.



## Dependencies### Core Optimization Tasks



**Core Requirements:**The challenge is divided into three core, interconnected optimization tasks:

- Python ≥3.8

- pyomo ≥6.7.0 (optimization framework)1. **Operation Optimization:** Maximize the BESS's revenue over the year 2024 by developing an optimal charge/discharge strategy. This involves performing energy arbitrage on the day-ahead wholesale market and placing capacity bids on the Frequency Containment Reserve (FCR) and automatic Frequency Restoration Reserve (aFRR) ancillary service markets.

- highspy ≥1.7.0 (HiGHS solver, open-source)

- pandas ≥2.0.0 (data processing)2. **Investment Optimization:** Identify which of five European countries (Germany, Austria, Switzerland, Hungary, Czech Republic) offers the highest Return on Investment (ROI) for installing the BESS over a 10-year period. This analysis must incorporate country-specific financial data, such as the Weighted-Average Cost of Capital (WACC) and inflation rates.

- numpy ≥1.24.0 (numerical computing)

- scipy ≥1.11.0 (financial calculations)3. **Configuration Optimization:** Determine the optimal BESS configuration by analyzing the impact of different C-rates and daily cycle limits on profitability and performance.
...
- cplex, gurobipy (commercial solvers, faster but not required)Submissions are evaluated based on:

- plotly, matplotlib (visualization, not needed for submission)1. **Revenue maximization (50%)**: How well the algorithm maximizes revenue based on market prices. A linear scaling formula will be used to map all submission values between the minimum and maximum observed.

2. **Investment optimization (20%)**: Assessment of optimal investment locations and markets.

## Output Format3. **Configuration optimization (20%)**: Assessment of the most relevant configuration parameters on BESS revenue.

4. **Code Quality and Documentation (10%)**: Clarity and structure of the code.

### Configuration.xlsx

Each country sheet contains:### Market Participation Rules

- C-rate: 0.25, 0.33, 0.50

- number of cycles: 1.0, 1.5, 2.0| Feature | Day-Ahead Market (EPEX SPOT) | FCR Capacity Market | aFRR Capacity Market |

- yearly profits [kEUR/MW]: Normalized annual revenue|---------|------------------------------|---------------------|----------------------|

- levelized ROI [%]: Simple ROI calculation| **Mechanism** | Blind Auction | Daily Auction | Daily Auction |

| **Gate Closure Time (D-1)** | 12:00 CET | 8:00 CET | 9:00 CET |

### Investment.xlsx| **Product Granularity** | 15 minutes | 4 hours | 4 hours |

Each country sheet contains:| **Bid Structure** | Energy (MWh) | Symmetric Capacity (MW) | Asymmetric Capacity (MW) |

- Financial parameters: WACC, inflation rate, discount rate| **Remuneration** | Pay-as-Cleared (Energy) | Pay-as-Cleared (Capacity) | Pay-as-Bid (Capacity) |

- 10-year cash flow: Initial investment and yearly profits (2023-2033)| **Minimum Bid Size** | 0.1 MW | 1 MW | 1 MW |

- Levelized ROI: DCF-based return on investment

### BESS Technical Parameters

### Operation.xlsx

Each country sheet contains full year schedule (35,136 rows):**Fixed Parameters:**

- Timestamp: 15-minute intervals (2024-01-01 00:00:00 to 2024-12-31 23:45:00)- Nominal Energy Capacity (E<sub>nom</sub>): 4,472 kWh (Huawei LUNA2000-4.5MWh)

- Stored energy [MWh], SoC [-]- Rated Power (P<sub>rated</sub>): 2,236 kW

- Charge [MWh], Discharge [MWh]- Initial Investment Cost (C<sub>inv</sub>): 200 EUR/kWh

- Day-ahead buy [MWh], Day-ahead sell [MWh]- Charging/Discharging Efficiency: 95%

- FCR Capacity [MW], aFRR Capacity POS [MW], aFRR Capacity NEG [MW]- SOC Range: 0-100% (full operational range allowed)



## Validation**Configuration Scenarios:**



Optional validation script for testing:| C-rate | Max Power (kW) | Daily Cycles | Max Daily Discharge (kWh) |

```bash|--------|----------------|--------------|---------------------------|

python validate_submission.py| 0.25 C | 1,118 | 1.0 | 4,472 |
...
- Import validation**Total Scenarios**: 6 countries (DE, DE_LU, AT, CH, HU, CZ) × 3 C-rates × 3 cycles = 54 scenarios

- File structure check

- Single scenario optimization---

- Investment analyzer test

- Solver detection## Mathematical Model
...
**Team:** SoloGen  The objective is to maximize the total net profit over the one-year horizon. This is the sum of day-ahead energy arbitrage revenue and ancillary service capacity payments, minus the cost of energy purchased for charging.

**Date:** October 2025  

**Python Version:** 3.8+  ```

**Solver:** HiGHS (open-source)max Z = Σ(t∈T) [(P_DA(t)/1000 · p_dis(t) - P_DA(t)/1000 · p_ch(t)) · Δt]
