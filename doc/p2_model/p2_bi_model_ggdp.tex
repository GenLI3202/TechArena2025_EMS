...
\begin{itemize}
  ...
  \begin{itemize}
    \item Bids $c(b)$ are in MW and prices $P(b)$ are in EUR/MW for each 4-hour block $b$.
  \end{itemize}
\end{itemize}
...
\subsubsection{Calculation of the Marginal Cost for Each SOC Segment $c^{\mathrm{cost}}_{j}$}
The marginal cost for discharging from each SOC segment is derived from the BESS's total investment cost and its expected lifetime. To ensure the model is physically-grounded and traceable, the cost is distributed non-linearly based on a standard power-law model for LFP battery degradation, rather than using non-traceable coefficients from literature examples.

\paragraph{Step 1: Select a Degradation Model}
For LFP batteries, the relationship between cycle life and Depth of Discharge (DoD) is commonly described by a power-law model:
\begin{equation*}
    \text{CycleLife}(D) = a \cdot D^{-b}
\end{equation*}
Where $D$ is the DoD, and for LFP batteries, the exponent $b$ is typically in the range [1.8, 2.2]. We adopt a standard, conservative value of $b=2$. The parameter $a$ represents the cycle life at 100\% DoD ($D=1$), which we assume to be 6,000 cycles.

\paragraph{Step 2: Derive Marginal Aging Weights}
The cumulative aging stress from discharging to a depth $D$ can be modeled as being proportional to $D^b$. Therefore, the marginal stress (or aging weight) $w_j$ incurred by discharging the $j$-th segment of the battery (from DoD $D_{j-1}$ to $D_j$) is:
\begin{equation*}
    w_j \propto D_j^b - D_{j-1}^b
\end{equation*}
With $b=2$ and 10 segments ($D_j = j/10$), the weight for each segment is proportional to $w_j \propto (j/10)^2 - ((j-1)/10)^2$. These weights are calculated and normalized. The sum of these weights, $W = \sum w_j$, conveniently equals 1.0, meaning they are already self-normalized.

\paragraph{Step 3: Calculate Final Marginal Costs}
With normalized weights $w_j$, the marginal cost for discharging 1 kWh from segment $j$ is calculated by allocating the `Cost per Full Cycle` (€149.07) according to these weights.
\begin{equation*}
c^{\mathrm{cost}}_{j} = \frac{\text{Cost per Full Cycle} \times w_j}{E_{\mathrm{nom}} \times \text{Segment Size}}
\end{equation*}
For example, for the first and shallowest segment ($j=1$), the marginal cost is calculated by plugging in the corresponding weight $w_1=0.01$:
\begin{equation*}
c^{\mathrm{cost}}_{1} = \frac{149.07 \text{ EUR/Cycle} \times 0.01}{4472 \text{ kWh} \times 0.1} \approx 0.0033 \text{ EUR/kWh}
\end{equation*}
Where the segment size is 0.1 (for 10\% segments). The resulting costs are detailed in Table \ref{tab:marginal_costs}.

\begin{table}[h!]
\centering
\caption{Calculation of Marginal Cyclic Aging Costs ($c^{\mathrm{cost}}_{j}$) based on a Power-Law Model ($b=2$)}
...
8 & 20-30\% & 0.15 & 0.0500 \\
9 & 10-20\% & 0.17 & 0.0567 \\
10 & 0-10\% & 0.19 & 0.0633 \\ \bottomrule
\end{tabular}
\end{table}
\textit{This approach ensures that the sum of costs incurred by discharging all 10 segments equals exactly one `Cost per Full Cycle` (€149.07), making the model economically consistent.}
...
% \subsubsection{Calculation of Calendar Aging Breakpoints}
% The breakpoints for the SOS2 linearization are derived from the "scaled" model presented in Collath et al. (2023), specifically from Figure 3, using the curve for 5\% past calendar capacity loss. The costs are monetized using the project's BESS investment cost of 200 EUR/kWh.

% \begin{table}[h!]
% \centering
% \caption{Calculation of Calendar Aging Breakpoints ($SOC^{\mathrm{point}}_i, Cost^{\mathrm{point}}_i$)}
% \label{tab:calendar_costs}
% \begin{tabular}{@{}cccc@{}}
% \toprule
% \textbf{Breakpoint ($i$)} & \textbf{SOC Level} & \textbf{SOC (`SOC_i^{\text{point}}`) [kWh]} & \textbf{Cost (`Cost_i^{\text{point}}`) [EUR/hr]} \\ \midrule
% 1 & 0\% & 0 & 1.76 \\
% 2 & 25\% & 1118 & 2.16 \\
% 3 & 50\% & 2236 & 3.60 \\
% 4 & 75\% & 3354 & 6.44 \\
% 5 & 100\% & 4472 & 10.72 \\ \bottomrule
% \end{tabular}
% \end{table}
% \textit{These values provide the concrete parameters for the SOS2 constraints (Eq. 44-47), ensuring the model accurately penalizes holding the battery at high states of charge.}
...
\subsection{Validation Needed: Explicit Cascading Constraints (Physical Rule)}
To eliminate any ambiguity from the economic incentive model, we can introduce a set of explicit physical constraints to strictly enforce the 'Last-In, First-Out' (LIFO) behavior of the battery segments. While this approach increases model complexity, it ensures that the model's behavior aligns perfectly with physical reality under all conditions. This requires introducing two new sets of binary decision variables.

\subsubsection{New Binary Variables for Cascading Logic}
\begin{tabular}{@{}lll}
\toprule
Symbol & Description & Type \\
\midrule
$u^{\text{empty}}_{j}(t)$ & \textbf{(New)} 1 if segment $j$ is empty 
   at time $t$, 0 otherwise & Binary \\
$u^{\text{full}}_{j}(t)$  & \textbf{(New)} 1 if segment $j$ is full at
   time $t$, 0 otherwise & Binary \\
\bottomrule
\end{tabular}

\vspace{1em} % Add some vertical space
We also introduce a new parameter $M$, which is a sufficiently large constant (typically set to $P^{\mathrm{config}}_{\max}$), for use in the "Big-M" constraints.

\subsubsection{New Constraints}

\paragraph{1. Linking Constraints for Binary Variables}
First, we must link the new binary variables to the segment's state f
   charge, $e_{\mathrm{soc},j}(t)$.

If a segment's energy $e_{\mathrm{soc},j}(t)$ is 0, then $u^{\text{empty}}_{j}(t)$ must be 1:
\begin{equation}
    e_{\mathrm{soc},j}(t) \le E^{\mathrm{seg}}_{j} \cdot (1 - u^{\text{empty}}_j(t)) \qquad \forall t, \forall j
\end{equation}
\textit{(If $u^{\text{empty}}$ is 1, the right-hand side becomes 0, forcing $e_{\mathrm{soc},j}$ to be 0.)}

\vspace{1em}
If a segment's energy $e_{\mathrm{soc},j}(t)$ reaches its capacity $E^{\mathrm{seg}}_{j}$, then $u^{\text{full}}_{j}(t)$ must be 1:
\begin{equation}
    e_{\mathrm{soc},j}(t) \ge E^{\mathrm{seg}}_{j} \cdot u^{\text{full}}_j(t) \qquad \forall t, \forall j
\end{equation}
\textit{(Only when $e_{\mathrm{soc},j}$ is full (equal to $E^{\mathrm{seg}}_{j}$) can $u^{\text{full}}_j$ be 1.)}

\paragraph{2. Cascading Discharge Constraint}
This constraint dictates that: \textit{discharging from segment $j$ is only permitted if the adjacent shallower segment $j-1$ is empty.}

\begin{equation}
p^{\mathrm{dis}}_{j}(t) \le M \cdot u^{\text{empty}}_{j-1}(t)    \qquad \forall t, \forall j > 1
\end{equation}
\textit{(This means: power $p^{\mathrm{dis}}_{j}$ can only be drawn  
      from segment $j$ if segment $j-1$ is empty ($u^{\text{empty}}_{j-1}=1$).)}

\paragraph{3. Cascading Charge Constraint}
This constraint dictates that: \textit{charging into segment $j$ is only permitted if the adjacent deeper segment $j+1$ is full.}
\begin{equation}
p^{\mathrm{ch}}_{j}(t) \le M \cdot u^{\text{full}}_{j+1}(t) \qquad \forall t, \forall j < J
\end{equation}
\textit{(This means: power $p^{\mathrm{ch}}_{j}$ can only be sent to segment $j$ if segment $j+1$ is full ($u^{\text{full}}_{j+1}=1$).)}
