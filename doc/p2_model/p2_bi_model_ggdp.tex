...
Additionally, to enforce the physical stack-like behavior of the segments, a simple monotonic ordering constraint is added. This ensures that a deeper segment (e.g., $j+1$) can only hold energy if the shallower segment above it ($j$) is full. This is a linear, non-binary alternative to the complex cascading constraints discussed in Section 4.8.
\begin{align}
...
(Details refer to `doc\p2_model\aging_cost\cyclic_aging.tex`)
...
\textbf{NOTICE: THIS SECTION IS CURRENTLY STRONGLY DIS-ENCOURAGED FOR IMPLEMENTATION DUE TO INCREASED MODEL COMPLEXITY. IT IS PROVIDED HERE FOR COMPLETENESS AND FUTURE REFERENCE.}
